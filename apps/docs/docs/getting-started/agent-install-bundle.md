...
For air-gapped or change-controlled environments you can download a **zip bundle** from the Infrawatch UI, transfer it to a target host, and install the agent without any network access to the Infrawatch server at install time.
...
| File | Purpose |
| --- | --- |
| `infrawatch-agent[.exe]` | The agent binary for the selected OS / arch |
...
1. Sign in to the Infrawatch web UI.
...
```sh
unzip infrawatch-agent-linux-amd64.zip
cd infrawatch-agent-linux-amd64
sudo ./install.sh
```
...
```sh
export INFRAWATCH_ORG_TOKEN="<token-from-ui>"
sudo -E ./install.sh
```
...
```powershell
$env:INFRAWATCH_ORG_TOKEN = "<token-from-ui>"
.\install.ps1
```
...
# Windows (PowerShell)
Get-FileHash -Algorithm SHA256 infrawatch-agent-windows-amd64.exe
```

## After install

The agent registers against the Infrawatch server and appears in **Settings → Agents** as `pending` — unless the embedded token has auto-approve enabled. An admin approves the agent from the UI; heartbeats start flowing immediately after approval.
